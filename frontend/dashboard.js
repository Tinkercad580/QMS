...
function renderKPI(d) {
    ...
    $('kWalkins').textContent = fmtNum(d.total_walkins);
    $('kWalkinsSub').textContent = `Walk-in queue entries`;
...
async function loadDaySnapshot(date) {
    try {
        ...
        // Revenue is now sum of collected (as requested)
        $('dRevenue').textContent = fmtMoney(d.collected);
        $('dPatients').textContent = fmtNum(d.patients);
        ...
        // New fields: Pending and New Registrations
        // Assuming d.pending and d.new_reg are passed by your backend /day route
        ...
        <tr>
          ...
          <td>
            <span class="patient-link" data-pid="${r.patient_id}" style="font-weight:600;cursor:pointer;color:var(--primary)">
